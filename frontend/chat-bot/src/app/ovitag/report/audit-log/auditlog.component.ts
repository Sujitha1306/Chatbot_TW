...
export class AuditlogComponent implements OnInit {
  ...
  displayedColumns: string[] = ['Create Time', 'URL', 'Host', 'User Id', 'Facility Id', 'Operation', 'Application', 'Message', 'Component Name', 'Table Name', 'Additional Info'];
  columnData: string[] = ['auditTimestamp', 'url', 'host', 'userId', 'facilityId', 'operation', 'application', 'message', 'componentName', 'tableName', 'additionalInfo'];
  ...
  getData(errorMessageLogdata, additonalInfoLogdata, stackTraceLogdata) {
    this.dialog.open(AuditLogdataComponent, {
  ...
  getAllLog(pageStart: number, pageSize: number) {
    ...
    // this.reportService.getAuditLogReport(this.fromDate, this.toDate, this.operation, this.pageStart, this.pageSize).subscribe((res) => {
    ...
    // return event;
     this.isloading = true;
    this.reportService.getAuditLogReport(this.fromDate, this.toDate, this.operation, pageStart, pageSize).subscribe(res => {
  ...
  downloadPDF() {
    const name = 'audit_log';
    this.chartImage = [];
    this.pdfService.exportAsPdfFile(this.exportData, this.displayedColumns, name, this.fromDate, this.toDate, this.chartImage);
  }
  fixClick() {
    console.log('')
  }  
...
@Component({
  selector: 'app-audit-logdata',
  templateUrl: './audit-logdata.component.html',
  styleUrls: ['./auditlog.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class AuditLogdataComponent {
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: any,
    private readonly dialogRef: MatDialogRef<AuditLogdataComponent>, public dialog: MatDialog) { }
