...
import { ManageChanneltemplateComponent } from './manage-channeltemplate/manage-channeltemplate.component';
...
export class ChannelTemplateComponent {
  ...
  getDynamicTableColumn() {
    this.commonService.getDynamicTableColumn('channel-template').subscribe((res) => {
      if (res.statusCode === 1) {
        ...
        if (this.tableVersion === 2) {
          const idIndex = this.displayedColumns.indexOf('ID');
          if (idIndex !== -1) {
            this.displayedColumns.splice(idIndex, 1);
            this.columnData.splice(idIndex, 1);
          }
        }
      ...
      this.getChannelTemp(this.pageStart, this.pageSize)
    });
  }

  getChannelTemp(pageStart: number, pageSize: number, name?: string) {
    this.isloading = true;
    this.commonService.getChannelTemplate().subscribe(res => {
  ...
  applyFilter(filterValue: string) {
    ...
    if (this.applyFilterValue.length > 2) {
      this.getChannelTemp(this.pageStart, this.pageSize, this.applyFilterValue);
    } else if (this.applyFilterValue.length == 0) {
      this.getChannelTemp(this.pageStart, this.pageSize, null);
    }
  }
  headerEventAction(event) {
    if (event.key === 'applyFilter') {
      ...
      this.createChannelTemp(null);
    } else if (event.data === 'Modify') {
      this.createChannelTemp(event.keyVal);
    } else {
  ...
  eventAction(event) {
    if (event.key === 'Name') {
      this.createChannelTemp(event.data);
  ...
  get channelTemplateTwColumns(): TwColumnDef[] {
  ...
  get channelTemplatePaginationConfig(): TwPaginationConfig {
  ...
  createChannelTemp(data: any) {
    this.showActions = null;
    this.selectedName = data;
    const dialogRef = this.dialog.open(ManageChanneltemplateComponent, {
      data: data,
