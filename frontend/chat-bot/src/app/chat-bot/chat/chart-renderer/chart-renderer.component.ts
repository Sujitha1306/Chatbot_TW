...
import { ChartType } from '../../models/chat.model';
// @ts-ignore
...
@Component({
  ...
  template: `
    <div *ngIf="!hasValidData" class="text-center py-12 text-gray-400 text-sm">📊 Not enough data for a {{ type }} chart.<br/>Try the Data Table view instead.</div>
    <div #chartContainer *ngIf="hasValidData"></div>`,
})
export class ChartRendererComponent implements OnChanges, AfterViewInit {
  ...
  private renderChart() {
    if (!this.hasValidData || !this.plotData.length) return;
    const el = this.chartContainer?.nativeElement;
    if (!el) return;
    this.needsRender = false;
    Plotly.newPlot(el, this.plotData, this.layout, { responsive: true, displaylogo: false });
  }
  ...
  @Input() spec?: any;
  @ViewChild('chartContainer') set container(content: ElementRef) {
    this.chartContainer = content;
    if (content && this.needsRender) {
