...
14. STABLE ORDERING WITH LIMIT: Whenever a query includes both ORDER BY and LIMIT, the ORDER BY must be fully deterministic — add a tie-breaking secondary sort column (e.g. the primary key or ID column) if the primary sort column could have duplicate values. This prevents ClickHouse from returning a different arbitrary subset of tied rows across repeated executions of the same query. IF the query uses LIMIT but does NOT have an ORDER BY, you MUST add an ORDER BY to ensure deterministic results.
